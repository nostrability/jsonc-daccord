                                            ...
                                            json_object *joutput_node)
{
    ...
    if (jmaxcontains) {
        ...
        if (match_count > max) {
            json_object *jmaxcont_node =
                _jdac_output_create_and_append_node(jcontains_node, "maxContains");
    ...
    if (jmincontains) {
        ...
        if (match_count < min && min != 0) {
            json_object *jmincont_node =
                _jdac_output_create_and_append_node(jcontains_node, "minContains");
