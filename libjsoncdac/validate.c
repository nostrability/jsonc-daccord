...
int _jdac_check_properties(json_object *jobj, json_object *jschema, json_object *joutput_node)
{
    ...
    json_object_object_foreach(jprops, jprop_key, jprop_val)
    {
        ...
        // printf("iobj %s type %d\nkey %s\nval %s\n", json_object_get_string(iobj),
        // json_object_get_type(iobj), jprop_key, json_object_get_string(jprop_val));
        if (iobj) {
...
int _jdac_check_uniqueItems(json_object *jobj, json_object *jschema, json_object *joutput_node)
{
    ...
    json_object *juniqueitems_node =
        _jdac_output_create_and_append_node(joutput_node, "uniqueItems");
        ...
        if (json_object_is_type(jmax, json_type_int) ||
            ...
            if (arraylen > maxitems) {
                json_object *jmaxitems_node =
                    _jdac_output_create_and_append_node(joutput_node, "maxItems");
        ...
        if (json_object_is_type(jmin, json_type_int) ||
            ...
            if (arraylen < minitems) {
                json_object *jminitems_node =
                    _jdac_output_create_and_append_node(joutput_node, "minItems");
...
int _jdac_validate_string(json_object *jobj, json_object *jschema, json_object *joutput_node)
{
    ...
    // printf("strlen of %s %ld %d %d\n", str, strlen(str), json_object_get_string_len(jobj),
    // utf8_length(str));
    ...
    if (jminlen) {
        ...
        if (utf8_length(str) < minlen) {
            ...
            json_object *jminlength_node =
                _jdac_output_create_and_append_node(joutput_node, "minLength");
    ...
    if (jmaxlen) {
        ...
        if (utf8_length(str) > maxlen) {
            ...
            json_object *jmaxlength_node =
                _jdac_output_create_and_append_node(joutput_node, "maxLength");
    ...
    int ret = minlength_ok == 1 && maxlength_ok == 1 && enums_ok == 1 && patterns_ok == 1
                  ? JDAC_ERR_VALID
                  : JDAC_ERR_INVALID;
    ...
    if (jmult) {
        ...
        if (multipland == 0.0) {
            json_object *jmultipleOf_node =
                _jdac_output_create_and_append_node(joutput_node, "multipleOf");
    ...
    if (jmin) {
        ...
        if (value < min) {
            ...
            json_object *jminimum_node =
                _jdac_output_create_and_append_node(joutput_node, "minimum");
    ...
    if (jexclmin) {
        ...
        if (value <= min) {
            ...
            json_object *jexclusiveMinimum_node =
                _jdac_output_create_and_append_node(joutput_node, "exclusiveMinimum");
    ...
    if (jmax) {
        ...
        if (value > max) {
            ...
            json_object *jmaximum_node =
                _jdac_output_create_and_append_node(joutput_node, "maximum");
    ...
    if (jexclmax) {
        ...
        if (value >= max) {
            ...
            json_object *jexclusiveMaximum_node =
                _jdac_output_create_and_append_node(joutput_node, "exclusiveMaximum");
            _jdac_output_apply_result(jexclusiveMaximum_node, JDAC_ERR_INVALID);
        }
    }
    int ret = multipleOf_ok == 1 && minimum_ok == 1 && exclusiveMinimum_ok == 1 &&
                      maximum_ok == 1 && exclusiveMaximum_ok == 1
                  ? JDAC_ERR_VALID
                  : JDAC_ERR_INVALID;
    // _jdac_output_apply_result(joutput_node, ret);
    return ret;
