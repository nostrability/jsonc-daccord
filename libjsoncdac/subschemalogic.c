...
enum subschematype { JDAC_ALLOF = 0, JDAC_ANYOF, JDAC_ONEOF };

// shall return valid or invalid based on subschema type
int _jdac_test_subschema_array(json_object *jobj, json_object *jsubschema_array,
                               enum subschematype type, json_object *joutput_node)
{
...
int _jdac_check_subschemalogic(json_object *jobj, json_object *jschema, json_object *joutput_node)
{
    ...
    if (if_schema) {
        ...
        if (err == JDAC_ERR_VALID && then_schema) {
            ...
            json_object *jelse_node =
                _jdac_output_create_and_append_node(joutput_node, "jnode_else");
    ...
    int subschemalogic_ret =
        allOf_ok == 1 && anyOf_ok == 1 && oneOf_ok == 1 && not_ok == 1 && if_ok == 1
            ? JDAC_ERR_VALID
            : JDAC_ERR_INVALID;
